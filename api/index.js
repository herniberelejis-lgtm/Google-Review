...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith('/r/')) {
      ...
      const piece = await redis.get(`piece:${code}`);
      ...
      await redis.incr(`stats:${code}:${source}`);
    ...
    if (pathname === '/api/admin/pieces') {
      ...
      if (req.method === 'POST') {
        ...
        req.on('end', async () => {
          ...
          await redis.set(`piece:${code}`, { name, link });
          res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (pathname === '/api/admin/stats') {
      ...
      for (let i = 1; i <= total; i++) {
        const code = String(i).padStart(3, '0');
        const qr = await redis.get(`stats:${code}:qr`) || 0;
        const nfc = await redis.get(`stats:${code}:nfc`) || 0;
        const web = await redis.get(`stats:${code}:web`) || 0;
